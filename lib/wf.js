...
const checkIsConnected = () => {
  return new Promise((resolve) => {
    request(SUCCESS_URL, (err, response, body) => {
      if (err) return resolve(false)
      resolve(body === SUCCESS_BODY)
    })
  })
}
...
module.exports.listValidMacs = async () => {
  ...
  const iface = os.networkInterfaces()[interface]
...
async function tryMACs(attemptedMACs, initialSSID, it) {
  const newMACs = (await module.exports.listValidMacs())
